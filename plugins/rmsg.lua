local function delmsg (i,naji)
    ...
    tg.getChatHistory(naji.messages_[0].chat_id_, naji.messages_[0].id_,0 , 40, delmsg, {msgs=msgs})
end
    ...
    if matches[1] == 'rmsg' and is_owner(msg) or is_momod(msg)  then
        if addgroup then 
            if tonumber(matches[2]) > 40 or tonumber(matches[2]) < 1 then
				...
				tg.getChatHistory(msg.chat_id_, 0, 0, tonumber(matches[2]), delmsg, cmd)
