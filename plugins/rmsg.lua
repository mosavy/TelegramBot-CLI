local function delmsg (i,naji)
    ...
    for k,v in pairs(naji.messages_) do
        ...
        if msgs == 1 then
            tg.deleteMessages(naji.messages_[0].chat_id_,{[0] = naji.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tg.getChatHistory(naji.messages_[0].chat_id_, naji.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
